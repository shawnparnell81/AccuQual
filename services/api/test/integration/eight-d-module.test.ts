...
async function makeUser(department: string | null) {
  const [user] = await db.insert(users).values({ tenantId, email: `eight-d-${department ?? "none"}-${suffix}-${Math.random().toString(36).slice(2, 7)}@test.local`, passwordHash: "unused" }).returning();
...
describe("8D report module (real DB + real HTTP path)", () => {
  beforeAll(async () => {
    const [tenant] = await db.insert(tenants).values({ name: `8D Test Tenant ${suffix}`, code: `eight-d-${suffix}` }).returning();
    tenantId = tenant!.id;
  ...
  afterAll(async () => {
    ...
    await db.delete(eightD).where(eq(eightD.tenantId, tenantId));
