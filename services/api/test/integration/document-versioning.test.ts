...
// records, RBAC, company isolation, audit trail, legacy (pre-versioning) documents, and the retired one-step endpoints.
...
const events = async (docId: number) =>
  (await db.select().from(auditTrail).where(and(eq(auditTrail.entityType, "DocumentVersion"), eq(auditTrail.entityId, docId)))).map((r) => r.changes as Record<string, unknown> | null);
...
describe("Controlled documents: draft -> review -> publish (real DB + real HTTP path)", () => {
  beforeAll(async () => {
    ...
    await db.update(users).set({ roleId: await roleId("admin") }).where(eq(users.id, admin.id));
    
    caliperId = (await db.insert(equipment).values({ name: "Digital caliper", serialNumber: "DC-100" }).returning())[0]!.id;
    acmeId = (await db.insert(suppliers).values({ name: "Acme Fasteners" }).returning())[0]!.id;
  ...
  describe("the whole lifecycle of one document", () => {
    ...
    it("sends for review with a named reviewer, tells that reviewer, and locks the draft", async () => {
      ...
      const mail = await db.select().from(notificationLog).where(and(eq(notificationLog.recipient, reviewer.email)));
    ...
    it("tells the department when a revision is sent for review, and the author when it is sent back", async () => {
      ...
      const deptMail = await db.select().from(notificationLog).where(and(eq(notificationLog.recipient, reviewer.email)));
      ...
      const authorMail = await db.select().from(notificationLog).where(and(eq(notificationLog.recipient, author.email)));
  ...
  describe("documents that pre-date versioning, and the retired one-step endpoints", () => {
    ...
    it("turns a released legacy document into version N, with its uploaded PDF registered as the file", async () => {
      const dir = path.join(env.STORAGE_LOCAL_PATH, "forms", "custom", "document-versions");
      await mkdir(dir, { recursive: true });
      const legacyPath = path.join(dir, `legacy-${suffix}.pdf`);
      await writeFile(legacyPath, PDF);
      const [doc] = await db.insert(documents).values({ title: "Legacy SOP", category: "SOP", status: "approved", currentVersion: 2, ownerId: author.id }).returning();
      const [ver] = await db.insert(documentVersions).values({ documentId: doc!.id, version: 2, fileUrl: legacyPath, approvedBy: reviewer.id, approvedAt: new Date(), createdBy: author.id }).returning();
    ...
    it("bootstraps a legacy document that was never released as an open draft, not as a released version", async () => {
      const [doc] = await db.insert(documents).values({ title: "Never released", status: "draft", ownerId: author.id }).returning();
      const c = await current(doc!.id, author);
      expect(c.published).toBeNull();
    ...
    it("refuses to stream any file outside the organization's own storage folder (an old endpoint accepted any path)", async () => {
      ...
      try {
        const [doc] = await db.insert(documents).values({ title: "Hostile path", status: "approved" }).returning();
        const [ver] = await db.insert(documentVersions).values({ documentId: doc!.id, version: 1, fileUrl: outside }).returning();
        expect((await request(app).get(`/documents/version/${ver!.id}/file`).set(as(author))).status).toBe(404);
        const [ver2] = await db.insert(documentVersions).values({ documentId: doc!.id, version: 2, fileUrl: "/etc/hosts" }).returning();
