...
// only existing coverage was company-isolation-modules.test.ts exercising a
// single field's isolation, never the real
...
async function makeUser(department: string | null) {
  const [user] = await db.insert(users).values({ email: `capa-workflow-${department ?? "none"}-${suffix}-${Math.random().toString(36).slice(2, 7)}@test.local`, passwordHash: "unused" }).returning();
