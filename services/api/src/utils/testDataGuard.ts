 ...
 * outside development" check (platform.service.ts's tenant provisioning,
 ...
 *
 * Matches the exact naming convention the ~26 cleaned-up test tenants and
 * the 4 cleaned-up test SKUs actually used (see scripts/cleanup-test-
 * tenants.ts and scripts/cleanup-test-skus.ts) — a "-test-<digits>" suffix
 * (vitest's own tenant-code convention, always followed by a timestamp) or
 ...
 */
const TEST_DATA_PATTERN = /-test-\d|test tenant|(^|[-_])(test|perf-test)([-_]|$)/i;
