...
export async function syncComplaintRecordToForm(db: TenantDb, tenantId: number, c: Complaint, createdBy?: number): Promise<void> {
  const patch: Record<string, unknown> = {};
  if (c.description) patch.description = c.description;
  if (c.resolution) patch.resolution = c.resolution;
  if (Object.keys(patch).length === 0) return;
  await mergeFormData(db, tenantId, { formType: COMPLAINT_FORM_TYPE, entityType: "complaint", entityId: c.id, patch, createdBy });
}
...
export async function syncComplaintFormToRecord(db: TenantDb, tenantId: number, id: number, data: Record<string, unknown>, performedBy?: number): Promise<void> {
  const [record] = await db.select().from(complaints).where(and(eq(complaints.id, id), eq(complaints.tenantId, tenantId)));
  ...
  await recordAuditTrail(db, { tenantId, entityType: "Complaint", entityId: id, action: "update", changes: { fieldsChanged: Object.keys(update), source: "form" }, performedBy });
}
