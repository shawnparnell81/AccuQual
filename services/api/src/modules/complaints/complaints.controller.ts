...
export const baseHandlers = crudFactory(complaints, {
  ...
  afterCreate: async (created, req) => {
    await syncComplaintRecordToForm(req.db!, req.tenantId!, created as unknown as Complaint, req.user?.id);
  },
});

async function loadOwned(req: Request): Promise<Complaint> {
  const [row] = await req.db!.select().from(complaints).where(and(eq(complaints.id, Number(req.params.id)), eq(complaints.tenantId, req.tenantId!)));
  if (!row) throw AppError.notFound("Complaint");
  return row;
}

async function assertNcrInTenant(req: Request, ncrId: number): Promise<void> {
  const [row] = await req.db!.select({ id: ncr.id }).from(ncr).where(and(eq(ncr.id, ncrId), eq(ncr.tenantId, req.tenantId!), eq(ncr.isDeleted, false)));
...
export const updateHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [updated] = await req
    ...
    .where(and(eq(complaints.id, current.id), eq(complaints.tenantId, req.tenantId!)))
    .returning();
  await recordAuditTrail(req.db!, { tenantId: req.tenantId!, entityType: "Complaint", entityId: current.id, action: "update", changes: req.body, performedBy: req.user?.id });
  await syncComplaintRecordToForm(req.db!, req.tenantId!, updated!, req.user?.id);
  res.json(updated);
});
...
async function transition(
  ...
  const [updated] = await req
    ...
    .where(and(eq(complaints.id, current.id), eq(complaints.tenantId, tenantId)))
    .returning();
  ...
  await publishEvent(WORKFLOW_STREAM, { tenantId, module: "complaints", event: opts.event, entityId: current.id });
  await syncComplaintRecordToForm(req.db!, tenantId, updated!, req.user?.id);
  res.json(updated);
}
...
export const escalateToNcrHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  if ((await getUserAccessLevel(req.db!, tenantId, req.user!, "ncr")) !== "edit") {
  ...
  const [updated] = await req
    ...
    .where(and(eq(complaints.id, current.id), eq(complaints.tenantId, tenantId)))
    .returning();
  await recordAuditTrail(req.db!, { tenantId, entityType: "Complaint", entityId: current.id, action: "update", changes: { action: "escalate_to_ncr", linkedNcrId: createdNcr!.id }, performedBy: req.user?.id });
