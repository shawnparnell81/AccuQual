...
export const createScarFormHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  await publishEvent(WORKFLOW_STREAM, { tenantId: req.tenantId!, module: "scar", event: "create", entityId: created!.id });
...
export const updateScarFormHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  await publishEvent(WORKFLOW_STREAM, { tenantId: req.tenantId!, module: "scar", event, entityId: record.id });
  res.json(updated);
});
