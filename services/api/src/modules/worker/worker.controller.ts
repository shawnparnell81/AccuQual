...
export const listWorkersHandler = asyncHandler(async (req: Request, res: Response) => {
  res.json(await service.listWorkers(req.db!, req.tenantId!));
});
...
export const getMyWorkerHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [profile, activity] = await Promise.all([service.getOwnWorkerProfile(req.db!, req.tenantId!, userId), service.getOwnWorkerActivity(req.db!, req.tenantId!, userId)]);
...
export const getWorkerHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [profile, activity] = await Promise.all([service.getWorkerProfile(req.db!, req.tenantId!, userId), service.getWorkerActivity(req.db!, req.tenantId!, userId)]);
  res.json({ profile, activity });
});
...
export const upsertWorkerHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const [existedBefore, before] = await Promise.all([service.profileRowExists(req.db!, req.tenantId!, userId), service.getWorkerProfile(req.db!, req.tenantId!, userId)]);
  const profile = await service.upsertWorkerProfile(req.db!, req.tenantId!, userId, input, req.user!.id);
