...
export const listPresetsHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json(await erpPresets.listPresets(req.db!, req.tenantId!, { vendor, module }));
});

export const getPresetHandler = asyncHandler(async (req: Request, res: Response) => {
  res.json(await erpPresets.getPreset(req.db!, req.tenantId!, Number(req.params.id)));
});
...
export const clonePresetHandler = asyncHandler(async (req: Request, res: Response) => {
  const cloned = await erpPresets.clonePreset(req.db!, req.tenantId!, Number(req.params.id), req.user?.id);
  res.status(201).json(cloned);
});

export const updatePresetHandler = asyncHandler(async (req: Request, res: Response) => {
  res.json(await erpPresets.updatePreset(req.db!, req.tenantId!, Number(req.params.id), req.body, req.user?.id));
});

export const deletePresetHandler = asyncHandler(async (req: Request, res: Response) => {
  await erpPresets.softDeletePreset(req.db!, req.tenantId!, Number(req.params.id), req.user?.id);
  res.status(204).send();
});

export const activatePresetHandler = asyncHandler(async (req: Request, res: Response) => {
  res.json(await erpPresets.activatePreset(req.db!, req.tenantId!, Number(req.params.id), req.user?.id));
});
