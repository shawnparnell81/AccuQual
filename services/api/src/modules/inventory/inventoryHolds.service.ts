...
export async function stockTotal(db: Db, itemId: number): Promise<number> {
...
export async function holdableUnits(db: Db, target: HoldTarget): Promise<number> {
...
async function bump(db: Db, target: HoldTarget, delta: number) {
  const [item] = await db.select({ held: inventoryItems.heldQty }).from(inventoryItems).where(and(eq(inventoryItems.id, target.itemId)));
  if (!item) throw AppError.notFound("Inventory item");
...
export async function placeHold(db: Db, target: HoldTarget, quantity: number, context: { quarantineId: number }, performedBy?: number): Promise<void> {
...
export async function liftHold(db: Db, target: HoldTarget, quantity: number, context: { quarantineId: number; reason: string }, performedBy?: number): Promise<void> {
  await bump(db, target, -quantity);
 ...
 */
export async function removeHeldStock(db: Db, target: HoldTarget, quantity: number, opts: { movementType: "scrap" | "return"; reason: string; quarantineId: number }, performedBy?: number): Promise<void> {
