...
function issueTokens(user: { id: number; tenantId: number | null; roleId: number | null; roleName: string | null; tokenVersion: number }) {
  const accessToken = signAccessToken({ sub: String(user.id), tenantId: user.tenantId, roleId: user.roleId, roleName: user.roleName });
...
export async function login(input: { email: string; password: string }) {
  ...
  const tokens = issueTokens({ id: user.id, tenantId: user.tenantId, roleId: user.roleId, roleName, tokenVersion: user.tokenVersion });
