 ...
 * entry (forms.validation.ts) with no layout registered for it at all, so
 * it fell all the way through to pdf-merger.ts's plain key:value dump —
 * the only form type in the Audits family (audit_plan/lpa both have real
 * structured layouts) left that way.
 *
 ...
 * present them as if copied from a real form (the audit's own review would
 * flag exactly that), this models the general shape every checklist-style
 * audit document in this app already shares (header, a compliance-response
 * table, a findings table, sign-off) and leaves the checklist rows
 * addable/blank, same treatment as Internal Audit Plan's own AGENDA/
 * ATTENDANCE tables for genuinely variable, per-audit content — not a
 * substitute for a real template if/when the user provides one.
 */
