/**
 * First-run guided checklist for a brand-new company's first admin, shown on
 ...
 *      for every EXISTING company, so nobody at a company that's been live
 *      for years suddenly sees a "new company" checklist.
 *   2. platform.service.ts's createCompany() — seeds a fresh, incomplete
 *      checklist for a genuinely new company.
 *
 * Each item's `key` is stored in `companies.onboardingProgress.completedItems`
 * — add a new item here by appending a key; removing one is safe too (a
 * stale completed key some company already has just becomes inert).
 */
