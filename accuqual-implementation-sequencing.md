...
Layer 0 — Platform foundation (must exist before anything else)
  tenants + RLS  →  users + roles  →  departmentAccess.ts (RBAC primitives: ResourceKey, getUserAccessLevel)
        │
        ├──────────────────────────────────────────────────────────────────────┐
        ▼                                                                      ▼
...
Layer 4b — AI Gateway (independent of Layer 4, NOT a dependency of it)
  llm-gateway.ts + ai.guardrails.ts (Zod schemas) → ai.usage.ts (BYOK/usage-limit, needs Tenant config
...
2. **State model** — `workflow_definitions`/`workflow_runs` schema. Depends only on Layer 0 (tenants).
...
1. **Permission enforcement order** (bottom-up, per request): `requireAuth` → `withTenantDb` (RLS context) → `requireDepartmentAccess(resourceKey)` / `requireRole(role)` / inline `assertDepartment()` — this exact middleware order is load-bearing; reversing `withTenantDb` and `requireDepartmentAccess` would let a department check run before tenant scoping exists.
...
4. **AI conflict — safety mode granularity.** `safetyMode` (`standard`/`strict`) is a single tenant-wide setting, but the platform's AI touchpoints span very different risk profiles (Risk/FMEA severity suggestions vs. Reporting's descriptive summaries). A tenant that sets `strict` to protect high-stakes Risk suggestions also silently makes low-stakes Reporting summaries fail closed more often than necessary. Not a bug — a real coarseness tradeoff worth flagging for any future per-module AI settings work (explicitly out of scope for Phase 10's Admin Console, which deliberately kept this tenant-wide).
...
```
1.  Tenants + RLS + Users + Roles + RBAC primitives  (Layer 0)
