...
**Layer 2 — The generic Workflow Engine (Platform Admin's Workflow Builder, `workflow_definitions`/`workflow_runs`).** A genuinely separate, company-configurable automation layer: a definition is a graph of `trigger`/`condition`/`action` nodes (`{nodes[], edges[]}`), matched against the `event` field of whatever a module's own `publishEvent(WORKFLOW_STREAM, {module, event, entityId, companyId, ...})` call just emitted. It does **not** own or enforce any module's state machine — it *reacts* to state changes Layer 1 already made, and can then fire cross-cutting actions (`send_email`, `notify_department`, `notify_supplier`, `create_ncr`, `escalate_capa`, `assign_user`, `ai_suggestion`). A module with zero `publishEvent` calls (see Document Control's create/revise/archive paths) is simply invisible to this layer for those events — Layer 2 can only ever be as complete as Layer 1's event coverage.
...
> **Current state (supersedes the points below where they differ).** Document Control now runs on the shared draft → review → publish engine (`modules/versioning`, subject `document`; adapter and file/link logic in `modules/documents/documentVersioning.ts`). A revision is a frozen `controlled_versions` row (DB trigger) carrying content, metadata (revision code, effective/expiration date, retention), files (`document_files` registry, SHA-256 per file) and links to workflows/equipment/suppliers/NCRs/CAPAs/audits/training. Review is four-eyes (admin excepted, recorded); `document.view|edit|review|publish` map onto the `documents` resource plus a reviewer role, customers/suppliers refused, refusals audited. `documents.status` mirrors the released revision (`approved`) or, before first release, the open draft. Direct `status`/title/category/expiration edits via PATCH are rejected; the old one-step `POST /documents/:id/version[/upload]` and `/approve` return 410. Publishing still writes the legacy `document_versions` ledger row (history, retention age-out) and emits the same `documents.approved` event. Retire (`/obsolete`) is refused while a revision is open; retention/archive unchanged.
...
**3. Rules** — **no RBAC gate whatsoever** — `documents` is not in the `ResourceKey` union. Only `requireAuth` applies: any authenticated company user can create, revise, approve, or archive any document.
...
**4. Conditions** — `defectCategory` (from the linked Quality Inspection Report, read-only cross-reference) can gate whether an auto-created NCR fires at all (company-configurable allow-list).
...
**2. Transitions** — lot quantity/status changes via `receiveIntoLot` (upsert by company+item+lot-number) and `consumeFromLot` (clamped decrement, never negative); item `state` is a derived, always-recomputed value, not a guarded machine.
...
**Reality check:** there is **no invite/pending-approval state** anywhere — both `POST /auth/register` (self-service, requires a valid company code) and `POST /users` (admin-created) create an **immediately active** user. The only real "onboarding email" in the codebase fires once, for the very first admin of a brand-new company (Platform Admin's company-provisioning flow) — a regular in-company `POST /users` sends no email of any kind, sent or logged.
